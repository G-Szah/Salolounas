// scripts/update-rosmariini.mjs
// TUOTANTOVALMIS Rosmariini-päivitys: hakee Rosmariinin oman lounassivun,
// varmistaa tämän päivän otsikon ja kirjoittaa vain data/menus/rosmariini.json.
...
async function main() {
  ...
  const date = new Date();
  const items = parseRosmariini(htmlToLines(await response.text()), date);
  const output = {
    name: 'Lounasravintola Rosmariini',
    address: 'Joensuunkatu 13, 24100 Salo',
    website: SOURCE_URL,
    source: SOURCE_URL,
  ...
  writeFileSync('data/menus/rosmariini.json', JSON.stringify(output, null, 2), 'utf8');
  console.log(`Rosmariini päivitetty: ${items.length} ruokalajia (${output.targetDate}).`);
}
