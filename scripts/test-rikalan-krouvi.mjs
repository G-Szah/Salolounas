// scripts/test-rikalan-krouvi.mjs
// VAIN TESTI: hakee Rikalan Krouvin oman lounassivun ja tallentaa debug-tiedostot.
...
function parseRikalanKrouvi(lines, date) {
  ...
  // Tukee muotoja "Keskiviikko 26.8." ja "Keskiviikko 26.8"
...
async function main() {
  ...
  try {
    const response = await fetch(SOURCE_URL, {
      headers: { 'User-Agent': 'Mozilla/5.0 (Salolounas Rikalan Krouvi test; +https://github.com/G-Szah/Salolounas)' },
      signal: AbortSignal.timeout(20000),
    });
    if (!response.ok) throw new Error(`Rikalan Krouvi fetch epäonnistui: HTTP ${response.status}`);

    const html = await response.text();
    writeFileSync(`${DEBUG_DIR}/rikalan-krouvi-source.html`, html, 'utf8');

    const lines = htmlToLines(html);
    const parsed = parseRikalanKrouvi(lines, date);
    const expected = ['Päärynä', 'Porkkanarisotto', 'Härän paahtopaisti', 'Kuningatarbrita'];
    ...
    const output = {
      ...
      sampleLines: lines.slice(0, 300),
    };
    writeFileSync(`${DEBUG_DIR}/rikalan-krouvi-parsed.json`, JSON.stringify(output, null, 2), 'utf8');
