// scripts/test-rosmariini.mjs
// VAIN TESTI: hakee Rosmariinin oman lounassivun ja tallentaa debug-tiedostot.
// Ei koske data/menu.json-tiedostoon eikä tuotantosivun dataan.

import { mkdirSync, writeFileSync } from 'node:fs';

const SOURCE_URL = 'https://pitopalvelurosmariini.fi/lounas/';
...
function parseRosmariini(lines, date) {
  const dayName = WEEKDAYS[date.getDay()];
  const day = date.getDate();
  const month = date.getMonth() + 1;
  // Sivulla voi olla "Keskiviikko 26.8." tai "Keskiviikko 26.8"
...
async function main() {
  ...
  try {
    const response = await fetch(SOURCE_URL, {
      headers: { 'User-Agent': 'Mozilla/5.0 (Salolounas Rosmariini test; +https://github.com/G-Szah/Salolounas)' },
      signal: AbortSignal.timeout(20000),
    });
    if (!response.ok) throw new Error(`Rosmariini fetch epäonnistui: HTTP ${response.status}`);

    const html = await response.text();
    writeFileSync(`${DEBUG_DIR}/rosmariini-source.html`, html, 'utf8');

    const lines = htmlToLines(html);
    const parsed = parseRosmariini(lines, date);
    const expected = ['Butter chicken', 'Jauhelihamoussaka', 'Riisi', 'Mustikkakiisseli'];
    ...
    writeFileSync(`${DEBUG_DIR}/rosmariini-parsed.json`, JSON.stringify(output, null, 2), 'utf8');
